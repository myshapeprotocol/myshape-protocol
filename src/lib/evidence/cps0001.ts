 ...
 *
 * Batch-2C hardening:
 *  - Every temporal input (interval.start/end, signature.signedAt, expiresAt)
 *    must PARSE. Previously a malformed value became NaN and silently passed
 *    every comparison below (`NaN >= x === false`), so garbage meant "valid".
 *    The spec oracle (reference-verifier) already enforced this; the producers
 *    had drifted.
 *  - Receipts attest COMPLETED observation windows: "The issuer signed it at
 *    the end of the interval" (cps-0001-onboarding/01-understand-receipt.md).
 *    An interval whose end lies in the future cannot yet have been observed —
 *    reject it regardless of any self-reported signedAt.
 *  - `now` is injectable for deterministic boundary tests; callers omit it in
 *    production.
 */
...
/** V₆: Freshness.
 *
 * Batch-2C: an unparseable expiresAt fails closed (EXPIRED) instead of slipping
 * through NaN comparison as "never expires". Parity with reference verifier.
 * `now` is injectable for deterministic boundary tests.
 */
...
export function canonicalSigningPayload(receipt: Omit<ContinuityReceipt, "signature">): string {
  ...
  // Batch-2B hardening: beyond legacy identity/interval/evidence digests, the
  // signature now also covers protocolVersion, expiresAt (freshness), the
  // assertions, verdict, and references. All three producers emit this EXACT
  // flat payload, so their signatures interoperate.
  return [
    receipt.receiptId,
