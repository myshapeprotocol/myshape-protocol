...
const COMPARISON_DATA: ComparisonRow[] = [
  ...
  { dimension: "Data Storage", myShape: "On-device by default — research uploads anonymized pose data with consent", worldcoin: "Iris hash stored on-chain", civic: "Encrypted PII with third-party verifiers", trip: "Device-local with remote attestation" },
  ...
  { dimension: "AI Forgery Resistance", myShape: "Hypothesis via entropy gap (4D analysis)", worldcoin: "Hardware-level (Orb) + liveness detection", civic: "Depends on document issuer security", trip: "Theoretical framework; limited published benchmarks" },
  ...
  { dimension: "Protocol Maturity", myShape: "v1.0-RC1 — evaluation / research stage", worldcoin: "Production — 18M+ verified users", civic: "Production — enterprise KYC/AML", trip: "IETF draft stage — pre-production" },
  ...
  { dimension: "Token / Incentive", myShape: "None — protocol-native continuity only", worldcoin: "WLD token + UBI distribution", civic: "CVC token (governance + settlement)", trip: "None — protocol only" },
  { dimension: "Decentralization", myShape: "Data stays on-device by default — open-source protocol, research stage", worldcoin: "Foundation-governed; Orb operators centralized", civic: "Federated — verifier network with centralized governance", trip: "Specification-level decentralization; implementation TBD" },
...
const CATEGORIES = [
  { title: "The Core Difference", paragraphs: ["MyShape Protocol is a continuity layer, not an identity layer. While Worldcoin, Civic, and SpruceID answer 'who are you?', MyShape answers 'are you still you?' — a fundamentally different question that becomes critical in the age of AI agents.", "Worldcoin proves you are a unique human (once). Civic proves you possess a valid credential (per-transaction). MyShape measures continuity across time — not who you are, but whether you remained you. This distinction is not semantic — it is the difference between a passport photo and a continuous proven presence."] },
  { title: "Why Motion Over Static Recognition", paragraphs: ["Static recognition techniques (iris, fingerprint, face) are fundamentally limited: once compromised, they cannot be replaced. Your iris is your iris forever. Motion is generative — each verification produces a fresh cryptographic proof. The protocol enforces temporal uniqueness, raising the cost of replay; replay resistance remains under investigation.", "Furthermore, motion encodes intent. A face scan proves a face was present. A motion-signature proves a specific intentional movement was executed. This intentionality gap sets MyShape apart from static-recognition approaches."] },
  { title: "The Agent Economy Is Coming", paragraphs: ["Worldcoin, Civic, and most identity protocols are designed for a world where the primary digital actor is human. But the Agent Economy is already here — autonomous AI agents executing transactions, voting in DAOs, and managing digital assets on behalf of humans.", "MyShape natively supports both human presence verification AND AI agent continuity binding in a unified attestation layer. Human agents and AI agents coexist, interact, and verify each other through the same protocol — with layered evidence that raises the cost of prolonged impersonation."] },
];
      ...
      <main className="flex-1 relative">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-6" style={{ paddingTop: "8rem", paddingBottom: "6rem" }}>
          <div className="space-y-4 mb-12">
            ...
            <h1 className="text-2xl md:text-4xl font-light tracking-[0.06em] text-white leading-tight">Continuity vs Identity<br /><span className="text-[#90c8ff]">Comparison Matrix</span></h1>
