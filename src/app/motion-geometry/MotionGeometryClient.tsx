...
const SIM_STEPS = [
  ...
  { t: 0.5, timing: 0.38, noise: 0.85, freq: 0.12, bio: 0.68, score: 0.62 },
  { t: 1.0, timing: 0.45, noise: 0.91, freq: 0.18, bio: 0.74, score: 0.71 },
  { t: 1.5, timing: 0.41, noise: 0.87, freq: 0.14, bio: 0.72, score: 0.78 },
  { t: 2.0, timing: 0.44, noise: 0.89, freq: 0.16, bio: 0.73, score: 0.85 },
  { t: 2.5, timing: 0.40, noise: 0.86, freq: 0.13, bio: 0.70, score: 0.89 },
  { t: 3.0, timing: 0.43, noise: 0.90, freq: 0.17, bio: 0.75, score: 0.94 },
  { t: 3.5, timing: 0.39, noise: 0.84, freq: 0.11, bio: 0.69, score: 0.92 },
  { t: 4.0, timing: 0.42, noise: 0.88, freq: 0.15, bio: 0.71, score: 0.96 },
...
function Gauge({ label, value, max = 1, color = "#90c8ff" }: { label: string; value: number; max?: number; color?: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-white/20 text-[8px] tracking-[0.1em] w-12 shrink-0">{label}</span>
      <div className="flex-1 h-1.5 bg-white/[0.04] overflow-hidden">
        <div className="h-full transition-all duration-700 ease-out" style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${color}99, ${color}cc)`, boxShadow: `0 0 6px ${color}44` }} />
      </div>
      <span className="text-white/40 font-mono text-[9px] w-10 text-right">{(value * 100).toFixed(0)}%</span>
    </div>
  );
}
...
function WireframePreview() {
  ...
  useEffect(() => {
    ...
    let frame = 0;
    const joints = [
      [0.5, 0.18], [0.5, 0.22], [0.5, 0.26], // spine
      [0.38, 0.22], [0.26, 0.35], [0.2, 0.48], // L arm
      [0.62, 0.22], [0.74, 0.35], [0.8, 0.48], // R arm
      [0.42, 0.48], [0.42, 0.65], [0.38, 0.82], // L leg
      [0.58, 0.48], [0.58, 0.65], [0.62, 0.82], // R leg
    ];
    ...
    const draw = () => {
      ...
      // 画骨骼
      bones.forEach(([a, b]) => {
        const jx = (Math.sin(t * 2 + a * 0.5) * 3 + Math.cos(t * 1.3 + b) * 2);
        const jy = Math.cos(t * 1.8 + b * 0.5) * 2;
        const ax = joints[a][0] * c.width + jx * 0.3;
        const ay = joints[a][1] * c.height + jy * 0.3;
        const bx = joints[b][0] * c.width + jx * 0.3;
        const by = joints[b][1] * c.height + jy * 0.3;
        ctx.strokeStyle = `rgba(144,200,255,${0.15 + Math.sin(t + a) * 0.05})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(ax, ay);
        ctx.lineTo(bx, by);
      ...
      // 关节点
      joints.forEach((j, i) => {
        const jx = Math.sin(t * 2 + i * 0.5) * 3;
        const jy = Math.cos(t * 1.8 + i) * 2;
...
export default function MotionGeometryClient() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStep((s) => (s + 1) % SIM_STEPS.length);
    }, 2500);
    ...
    <div className="min-h-screen bg-[#02040a] text-[#f8feff] font-mono selection:bg-[#90c8ff]/30">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-6 pt-24 md:pt-28 pb-16">
        <div className="text-center mb-16">
          ...
          <p className="text-white/30 text-[12px] max-w-lg mx-auto leading-relaxed">
            Watch a simulated motion pipeline transform raw skeletal data into entropy scores and a cryptographic identity vector.
          </p>
        </div>
