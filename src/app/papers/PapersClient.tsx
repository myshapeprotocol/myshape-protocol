              ...
              <div key={paper.id} className="group relative border border-white/5 bg-white/[0.02] p-8 hover:border-cyan-500/30 transition-all duration-500">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-6 relative z-10">
                  ...
                  <div className="flex flex-col items-end gap-6 min-w-[120px]">
                    ...
                    <a href={paper.id === "PAPER_01" ? "/papers/core-protocol" : "#"} className={`text-[10px] tracking-[0.4em] uppercase font-bold transition-all flex items-center gap-2 group/btn ${paper.id === "PAPER_01" ? "text-cyan-500 hover:text-white" : "text-white/10 cursor-default"}`}>
                      <span>{paper.id === "PAPER_01" ? "READ_PAPER" : "FORTHCOMING"}</span>
                      {paper.id === "PAPER_01" && <span className="group-hover/btn:translate-x-1 transition-transform">→</span>}
                    </a>
                  </div>
                </div>
              </div>
