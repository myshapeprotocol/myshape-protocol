      ...
      <div className="space-y-32">
        <section className="space-y-16">
          <div className="max-w-4xl">
            ...
            <p className="text-xl md:text-2xl font-light tracking-[0.2em] text-white leading-tight uppercase">FOR PARTNERSHIPS, RESEARCH COLLABORATION, OR <span className="text-[#90c8ff]">PROTOCOL INTEGRATION</span>.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="ct-card" onMouseEnter={() => playTick(500, "sine", 0.04, 0.01)}>
              ...
              <p className="text-white/40 text-[10px] tracking-[0.2em] leading-loose uppercase italic">"MYSHAPE MAINTAINS A STRICT PRIVACY-FIRST COMMUNICATION POLICY."</p>
            </div>
            <div className="ct-card flex flex-col justify-center" onMouseEnter={() => playTick(500, "sine", 0.04, 0.01)}>
              <span className="block text-white/20 text-[8px] tracking-[0.4em] uppercase mb-2">Contact</span>
              <a href="mailto:protocol@myshape.com" className="ct-email" onMouseEnter={() => playTick(600, "sine", 0.06, 0.015)}>PROTOCOL@MYSHAPE.COM</a>
            </div>
          </div>
        </section>
        ...
        <section id="nodes" className="space-y-12 pt-16 border-t" style={{ borderColor: "rgba(144,200,255,0.1)" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {CONNECT_NODES.map((node) => (
              <a key={node.id} href={node.url} target="_blank" rel="noopener noreferrer" className="ct-node-card" onMouseEnter={() => playTick(700, "sine", 0.08, 0.015)}>
        ...
        <div className="py-12 text-center border-b" style={{ borderColor: "rgba(144,200,255,0.08)" }}>
          <p className="text-white/20 text-[9px] tracking-[0.4em] uppercase leading-loose font-light italic">DECENTRALIZED NODES ACTIVE // NO PHYSICAL HEADQUARTERS // ZERO_POINT_FAILURE_PROTOCOL</p>
        </div>
      </div>
    </ProtocolLayout>
  );
