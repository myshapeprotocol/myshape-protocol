...
const pillars = [
  ...
  },
  {
    ...
    accent: "from-[#00E5FF]/60 to-[#00E5FF]/0",
  ...
  },
  {
    ...
    desc: "Your Motion-Signature is inscribed into a self-sovereign receipt. No server holds it. No platform controls it. No authority can revoke it. Sovereignty is not granted — it is mathematically enforced.",
    accent: "from-[#00E5FF]/45 to-[#00E5FF]/0",
  ...
  },
  {
    ...
    accent: "from-[#00E5FF]/35 to-[#00E5FF]/0",
    color: "rgba(0,229,255,0.25)",
  },
];

const futureStates = [
  { label: "TODAY", desc: "CPS-0001 v1.0-RC — the open continuity protocol. Four evidence engines and the npm SDK are public; seeking first independent verifiers.", active: true },
    ...
    >
      <div className="space-y-24 md:space-y-36">
        ...
        <section className="relative max-w-3xl"
          ...
          <div className="text-[#00E5FF]/[0.03] text-[140px] font-bold absolute -top-20 -left-10 select-none pointer-events-none leading-none">C</div>
          <div className="relative z-10 border-l-2 border-[#00E5FF]/25 pl-6 md:pl-10 py-3">
            <div className="text-[#00E5FF]/50 text-[11px] tracking-[0.3em] uppercase font-mono mb-5">// THE CONTINUITY THESIS</div>
            <p className="text-white/80 text-[15px] md:text-[18px] leading-[1.8] font-light mb-5">
              The digital world spent three decades answering <span className="text-[#00E5FF]/70">Who are you?</span>
            ...
            <p className="text-white/60 text-[13px] md:text-[15px] leading-[1.9] font-light">
              ...
              <span className="text-[#00E5FF]/90 font-medium">who continues to be you.</span>
        ...
        {/* ── 1. 协议立场 ── */}
        <section className="relative max-w-3xl">
          <div className="text-[#00E5FF]/[0.2] text-[90px] font-bold absolute -top-12 -left-6 select-none pointer-events-none">EYE</div>
        ...
        {/* ── 2. 四大支柱 ── */}
        <section>
          <div className="text-white/30 md:text-white/35 text-[11px] md:text-[12px] tracking-[0.5em] md:tracking-[0.6em] uppercase mb-12 text-center hover:text-[#00E5FF]/50 transition-colors cursor-default"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {pillars.map((pillar) => (
              <div key={pillar.id} className="group border p-6 transition-all duration-500"
                style={{ borderColor: "rgba(0,229,255,0.08)", background: "transparent" }}
                onMouseEnter={e => { playTick(600, "sine", 0.08, 0.015); e.currentTarget.style.borderColor = pillar.color; e.currentTarget.style.background = "rgba(0,229,255,0.02)"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(0,229,255,0.08)"; e.currentTarget.style.background = "transparent"; }}>
                <div className={`h-[2px] mb-5 bg-gradient-to-r ${pillar.accent} w-12 group-hover:w-full transition-all duration-700`} />
                <div className="text-[#00E5FF]/40 text-[11px] tracking-[0.4em] font-mono mb-2 group-hover:text-[#00E5FF]/70 transition-colors">
                  {pillar.id}
                </div>
                <h3 className="text-white/80 text-[13px] tracking-[0.25em] font-bold uppercase mb-2 group-hover:text-white transition-colors">
                  {pillar.title}
                </h3>
                <p className="text-[#00E5FF]/30 text-[11px] tracking-[0.15em] uppercase mb-4 group-hover:text-[#00E5FF]/50 transition-colors">
                  {pillar.subtitle}
                </p>
        ...
        <section className="border-t border-white/[0.04] pt-16">
          <div className="text-white/30 md:text-white/35 text-[11px] md:text-[12px] tracking-[0.5em] md:tracking-[0.6em] uppercase mb-14 text-center hover:text-[#00E5FF]/50 transition-colors cursor-default"
            onMouseEnter={() => playTick(450, "sine", 0.04, 0.022)}>
            Protocol Trajectory
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {futureStates.map((s, i) => (
              <div key={s.label} className="group text-center"
                ...
                <div className={`text-[12px] tracking-[0.3em] uppercase mb-5 font-mono font-bold transition-all duration-500 ${s.active ? "text-[#00E5FF]/70" : "text-white/25 group-hover:text-[#00E5FF]/50"}`}>
                  {s.label}
                </div>
                <div className={`h-[2px] mx-auto mb-5 transition-all duration-500 ${s.active ? "bg-[#00E5FF]/50 w-16" : "bg-white/10 w-8 group-hover:bg-[#00E5FF]/25 group-hover:w-14"}`} />
                <p className="text-white/35 text-[12px] leading-[1.8] group-hover:text-white/50 transition-colors">
                  {s.desc}
                </p>
                {s.active && (
                  <div className="mt-3 text-[#00E5FF]/30 text-[11px] tracking-[0.2em] uppercase">◈ ACTIVE</div>
                )}
              </div>
            ))}
          </div>
        ...
        <section className="py-20 border-y border-white/[0.04] relative overflow-hidden"
          ...
          <div className="max-w-xl mx-auto text-center space-y-8 relative z-10">
            <div className="flex items-center justify-center gap-3">
              <div className="h-[1px] w-8 bg-gradient-to-r from-transparent to-[#00E5FF]/20" />
              <span className="text-white/28 text-[11px] tracking-[0.4em] uppercase font-mono hover:text-[#00E5FF]/40 transition-colors cursor-default"
                onMouseEnter={() => playTick(450, "sine", 0.03, 0.022)}>Continuity_Projection_001</span>
              <div className="h-[1px] w-8 bg-gradient-to-l from-transparent to-[#00E5FF]/20" />
            </div>
            ...
            <div className="flex justify-center gap-3">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="w-2 h-2 bg-[#00E5FF]/20 rounded-full hover:bg-[#00E5FF]/50 transition-all duration-300 hover:scale-125 cursor-default"
        ...
        <section className="flex flex-col md:flex-row justify-between items-end gap-8">
          <div className="space-y-3">
            <div className="text-[#00E5FF]/55 text-[12px] tracking-[0.4em] font-bold uppercase">
              Status: Continuity_Layer_Active
            </div>
            <p className="text-white/30 text-[12px] tracking-[0.12em] max-w-sm leading-relaxed">
              CPS-0001 v1.0-RC is live — an open protocol seeking its first independent verifiers.
              Building the verification primitive for the Agent Economy.
            </p>
            <Link href="/protocol"
              onMouseEnter={() => playTick(800, "sine", 0.10, 0.025)}
              className="inline-block mt-4 px-10 py-3.5 border border-[#00E5FF]/30 text-[#00E5FF]/70 text-[12px] tracking-[0.3em] uppercase hover:bg-[#00E5FF]/[0.04] hover:text-white hover:border-[#00E5FF]/50 transition-all"
              style={{ clipPath: "polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)", background: "rgba(0,229,255,0.03)" }}>
