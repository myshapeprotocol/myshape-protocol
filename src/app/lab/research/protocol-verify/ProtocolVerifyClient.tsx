...
import { buildEvidenceReceiptAsync, type BuildReceiptResult } from "./lib/build-receipt";
...
export default function ProtocolVerifyClient() {
  ...
  const run = useCallback(async () => {
    ...
    if (dec === "reject") {
      ...
      // Phase A: Build CPS-0001 Receipt (non-extractable browser identity)
      const r1 = await buildEvidenceReceiptAsync({
    ...
    if (dec === "accept") {
      ...
      // Phase A: Build CPS-0001 Receipt (non-extractable browser identity)
      const rA = await buildEvidenceReceiptAsync({
        evidence: passiveEvidence,
        startTime: startWallRef.current,
        endTime: new Date(),
      });
    ...
    // Phase A: Build CPS-0001 Receipt (escalate path — dual-engine evidence, non-extractable identity)
    const r3 = await buildEvidenceReceiptAsync({
