...
for PATTERN in "${BANNED[@]}"; do
  while IFS= read -r file; do
    ...
    if [ -f "$file" ]; then
      ...
      MATCHES=$(echo "$RAW_MATCHES" | grep -v -i -E 'data.body|non.biometric|no[[:space:]]+biometric|Particle.Body|body[: ].|BodyInit|<body|</body|body \{|biometric, device attestation, reputation|biometric binding is enforced|Post Body$' || true)
      if [ -n "$MATCHES" ]; then
